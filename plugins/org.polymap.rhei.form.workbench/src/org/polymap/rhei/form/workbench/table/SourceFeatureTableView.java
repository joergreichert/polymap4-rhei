package org.polymap.rhei.form.workbench.table;
...
public class SourceFeatureTableView
    ...
    public static SourceFeatureTableView open( final FeatureSource fs ) {
        ...
        UIUtils.sessionDisplay().syncExec( new Runnable() {
            public void run() {
                try {
                    ...
                    StatusDispatcher.handleError( DataPlugin.PLUGIN_ID, null, e.getMessage(), e );
                }
                finally {
                    initFs.remove();
                }
    ...
    public void createPartControl( final Composite _parent ) {
        ...
        viewer.getTable().setLayoutData( new FormDataFactory().fill().create() );
