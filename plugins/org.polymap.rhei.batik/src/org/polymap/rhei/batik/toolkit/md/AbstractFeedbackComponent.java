...
public abstract class AbstractFeedbackComponent
    ...
    public AbstractFeedbackComponent( MdToolkit tk, Composite parent, int verticalPosition, int width, int style ) {
        comp = tk.createComposite( parent, style );
        int remainingWidthForEachSide = Integer.valueOf( (100 - width) / 2 );
        comp.setLayoutData( FormDataFactory.defaults().top( verticalPosition ).left( remainingWidthForEachSide ).right( 100 - remainingWidthForEachSide )
                .bottom( verticalPosition ).create() );
        comp.moveAbove( null );
